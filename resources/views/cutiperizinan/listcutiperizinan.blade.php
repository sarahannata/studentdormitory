...
<head>
    ...
    <title>Cuti/Perizinan Pegawai</title>
</head>
    ...
    <div class="flex h-screen">
        ...
        <div class="flex-1 p-6 fade-in">
            <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                <h1 class="text-3xl font-extrabold text-[#6B5DD3]">📅 Cuti/Perizinan Pegawai</h1>
            ...
            <div class="bg-white rounded-3xl shadow-xl p-6 hover:shadow-2xl transition duration-300 ease-in-out">
                <div class="flex justify-between items-center mb-6 flex-wrap gap-4">
                    ...
                    <div class="flex gap-5">
                        <form action="{{ route('cutiperizinan.admin') }}" method="GET" id="search-form">
                        ...
                        <tbody class="text-gray-600">
                            @forelse ($cutis as $cuti)
                                <tr class="border-b border-gray-200 transition duration-200 ease-in-out">
                                    ...
                                    <td class="py-3 px-4">
                                        @php $status = strtolower(trim($cuti->status)); @endphp
                                        @if ($status === 'dalam proses')
                                            <div class="flex justify-center gap-2">
                                                <form action="{{ route('cutiperizinan.setujui', $cuti->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    @include('components.buttonapprove')
                                                </form>
                                                <form action="{{ route('cutiperizinan.tolak', $cuti->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="py-6 text-gray-400 italic">😔 Belum ada pengajuan
                                        cuti/izin.</td>
                                </tr>
                            @endforelse
                        </tbody>
